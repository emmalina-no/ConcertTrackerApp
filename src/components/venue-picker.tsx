...
export function VenuePicker({
  ...
  const [mode, setMode] = useState<'selected' | 'search' | 'create'>(value.name ? 'selected' : 'search');
  ...
  if (mode === 'create') {
    return (
      <ThemedView style={styles.createContainer}>
        <ThemedTextInput value={value.name} onChangeText={(name) => onChange({ ...value, name })} placeholder="Venue name" />
        <ThemedTextInput value={value.city} onChangeText={(city) => onChange({ ...value, city })} placeholder="City" />
        <ThemedTextInput value={value.country} onChangeText={(country) => onChange({ ...value, country })} placeholder="Country" />
        <Pressable onPress={changeVenue}>
  ...
  return (
    <ThemedView style={styles.searchContainer}>
      <ThemedTextInput value={query} onChangeText={setQuery} placeholder="Search venues..." />
      {matches.length > 0 && (
        <ThemedView type="backgroundElement" style={styles.suggestions}>
          {matches.map((venue) => (
            <Pressable key={venue.id} onPress={() => selectVenue(venue)} style={styles.suggestionRow}>
      ...
      {query.trim().length > 0 && (
        <Pressable onPress={startCreate}>
          <ThemedText type="linkPrimary">+ Create new venue &quot;{query.trim()}&quot;</ThemedText>
        </Pressable>
      )}
    </ThemedView>
  );
